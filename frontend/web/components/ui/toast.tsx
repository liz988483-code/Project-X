...
const toastVariants = cva(
  'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full',
  {
    variants: {
      variant: {
        default: 'border bg-white dark:bg-gray-900',
        destructive:
        ...
        info: 'info group border-blue-500 bg-blue-50 text-blue-900 dark:border-blue-900 dark:bg-blue-900 dark:text-blue-50',
      },
    },
    defaultVariants: {
      variant: 'default',
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      'absolute right-2 top-2 rounded-md p-1 text-gray-500 opacity-0 transition-opacity hover:text-gray-900 focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 dark:text-gray-400 dark:hover:text-gray-50',
      className
    )}
    toast-close=""
    {...props}
...
function toast(options: ToastOptions) {
  ...
  const getVariant = (type: ToastType = 'default') => {
  ...
  const getIcon = (type: ToastType = 'default') => {
    ...
    return options.icon || iconMap[type]
  }
...
toast.warning = (options: Omit<ToastOptions, 'type'>) =>
  toast({ ...options, type: 'warning' })

toast.info = (options: Omit<ToastOptions, 'type'>) =>
  toast({ ...options, type: 'info' })

toast.loading = (options: Omit<ToastOptions, 'type'>) => {
  const id = toast({
    ...options,
    type: 'info',
    duration: Infinity,
    icon: <div className="h-5 w-5 animate-spin rounded-full border-2 border-gray-400 border-t-transparent" />,
  })
  return id
}

// Toast component for use in JSX
const ToastComponent: React.FC<ToastData> = ({
  title,
  description,
  action,
  type = 'default',
  ...
  const variant = {
    ...
    error: 'destructive',
    info: 'info',
  }[type] as VariantProps<typeof toastVariants>['variant']
